...
if (count($parts) !== 3) {
    echo "<script>alert('Invalid Google token.'); window.location.href='personal_info_form.php';</script>";
    exit;
}
...
if (!$payload) {
    echo "<script>alert('Failed to decode Google token.'); window.location.href='personal_info_form.php';</script>";
    exit;
}
...
if (!$email) {
    echo "<script>alert('Could not get email from Google.'); window.location.href='personal_info_form.php';</script>";
    exit;
}
...
if ($result->num_rows > 0) {
    // ── EXISTING USER: update photo and log in directly (Google already verified them) ──
    ...
    try {
        ...
        echo "<script>alert('Could not send OTP email. Please try again.'); window.location.href='personal_info_form.php';</script>";
        exit;
    }
}
?>